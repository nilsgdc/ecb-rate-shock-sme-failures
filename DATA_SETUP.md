...
matching `data/raw/` subfolder. A difficulty level is given per source: one-click,
manual export from a portal, or needs-a-choice (a format decision or conversion).

---

## 1. ECB policy rates (one-click) — `data/raw/bce_rates/`
...
facility −0.50% to 0.00%, main refi 0.00% to 0.50%. This is the DiD treatment date
...
## 2. Business failures by sector & size (manual export) — `data/raw/banque_de_france/`
...
## 3. Credit by sector (treasury vs total) (manual export) — `data/raw/banque_de_france/`
...
1. Select **outstanding credit by NAF sector**, split into **treasury/short-term
   credit** vs **investment credit** (their sum is approximately total).
2. Frequency = **monthly**, period **2018–2021** (the window used for the ratio,
   `INTENSITY_WINDOW` in `src/cleaning.py`).
3. Export to CSV, save as `credit_by_sector.csv`.

Caveat: the exact treasury-vs-total split at fine NAF granularity may not be
available for every sector. If so, we adapt the treatment-construction method
...
## 4. ZFRR municipality list (needs a choice) — `data/raw/zfrr/`

The territorial-equity classification (commune to ZFRR / ZFRR+).
...
The main data.gouv resource is a **shapefile** (.shp), not a flat CSV. Two options:
...
## 5. SME counts per sector (optional) — `data/raw/sirene/`

Used **only** to weight failure counts by the number of SMEs at risk per sector.

The full SIRENE registry is ~4 GB — overkill for sector-level weights (same lesson
as the ZFE project, where we filtered a 123 MB national shapefile down to 2.6 MB).
Prefer an **INSEE aggregated table**: number of enterprises by NAF sector and size
class (*Démographie des entreprises* / stock REE) from `https://www.insee.fr/`. A
few hundred KB instead of gigabytes.
...
| 2 | Failures by sector & size (Webstat) | `banque_de_france/` | Manual export | Dependent variable |
| 3 | Credit by sector (Webstat) | `banque_de_france/` | Manual export | Treatment group |
| 4 | ZFRR communes (data.gouv / OdT) | `zfrr/` | Shapefile | Territorial interaction |
| 5 | SME counts (INSEE) | `sirene/` | Optional | Weighting (robustness) |

Start with **1, then 2, then 3** (the DiD core). 4 and 5 add the equity dimension
and weighting.
